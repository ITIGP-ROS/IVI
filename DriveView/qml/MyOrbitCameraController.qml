...
Item {
    ...
    // ~6.8 deg at the chase distance of 1200. maxPitch() adds 1 deg of margin.
    property real minPitch: -90
    property real groundY: -141.5

    function maxPitch() {
        return Math.min(89, Math.asin(Math.min(1, -root.groundY / Math.max(root.camera.z, 1))) * 180 / Math.PI - 1)
    }
    ...
    WheelHandler {
        ...
        target: null
        enabled: root.mouseEnabled && root.zoomEnabled
    ...
    QtObject {
        ...
        function processInput(frameDelta) {
            if (useMouse) {
                ...
                if (root.yInvert)
                    rotateY = -rotateY;
                rotationVector.x = Math.max(root.minPitch,
                                            Math.min(rotationVector.x + rotateY,
                                                     root.maxPitch()));
